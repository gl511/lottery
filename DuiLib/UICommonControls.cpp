...
void CButtonUI::SetFocusState(bool bFocus)
{
	if(bFocus)
	{
...
void CButtonUI::SetPushState(bool bPush)
{
	if(bPush)
	{
...
CSize CTextUI::GetRealTextSize()
{
	RECT rcText = { 0, 0, m_cxyFixed.cx, 9999 };
	rcText.left += m_rcTextPadding.left;
	rcText.right -= m_rcTextPadding.right;
	if( m_bShowHtml ) {   
		int nLinks = 0;
...
BOOL CTextUI::EnableShowAllText()
{
	CSize sz = GetRealTextSize();
	if(sz.cy <= m_cxyFixed.cy)
		return TRUE;

...
//
//
	BOOL LoadLocalPictureToStream(LPCTSTR strPicPath, IStream *&pStream, DWORD &dwSize)
	{
		// read from file
		HANDLE hFile = CreateFile(strPicPath, GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		DWORD dwBytesRead = 0;
		BOOL bRead = ReadFile(hFile, pvData, dwSize, &dwBytesRead, NULL);
		if ( FALSE == bRead || dwBytesRead != dwSize )
		{
			GlobalUnlock(hGlobal);
			GlobalFree(hGlobal);
			CloseHandle(hFile);
	...
	HBITMAP LoadLocalPictureToBmp(LPCTSTR strPicPath)
	{
		BOOL BRet = FALSE;
		DWORD dwSize = 0;
		...
		BRet = LoadLocalPictureToStream(strPicPath, pStream, dwSize);
		if(FALSE == BRet)
		{
			return hBmpRes;
		}

		// ÏÔÊ¾jpeg ¡¢gif ¡¢bmp¡¢icoÍ¼Ïñ
		Gdiplus::Bitmap *pBitmap = Gdiplus::Bitmap::FromStream(pStream);
		if (pBitmap != NULL)
		{
			Gdiplus::Color crBgnd = Gdiplus::Color::Transparent;
			pBitmap->GetHBITMAP(crBgnd, &hBmpRes);
		}
		pStream->Release();
		pStream = NULL;
	...
	bool DrawAlpha(Image *pBgImg, HWND hWnd, BLENDFUNCTION *pBlend)
	{
		...
		::UpdateLayeredWindow(hWnd, hdcScreen, &ptWinPos, &sizeWindow, 
			hdcMemory, &ptSrc, 0, pBlend, ULW_ALPHA);
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR szFilePath, bool bAlpha)
{
	if ( szFilePath == NULL || _tcslen(szFilePath) == 0 ) return NULL;

	const TImageInfo* pdata = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
	if (pdata != NULL) 
	{
		return pdata;
	}

	pdata = AddImage(bitmap);
	if (pdata != NULL) 
	{
		return pdata;
	}

	HBITMAP hBitmap = LoadLocalPictureToBmp(szFilePath);
